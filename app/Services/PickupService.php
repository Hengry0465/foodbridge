...
class PickupService
{
    ...
    public function schedulePickup(array $data, User $user): Pickup
    {
        return DB::transaction(function () use ($data, $user) {
            $match = MatchRecord::with(['donation', 'foodRequest'])
            ...
            if ($match->foodRequest->recipient_id !== $user->id) {
            ...
            $pickup = Pickup::create([
                ...
                'donor_id' => $match->donation->donor_id,
                'recipient_id' => $match->foodRequest->recipient_id,
            ...
            ->where(function ($query) use ($startTime, $endTime) {
                $query->where(function ($q) use ($startTime, $endTime) {
                    $q->where('scheduled_at', '<', $endTime)
                        ->whereRaw(
                            "DATE_ADD(scheduled_at, INTERVAL {$this->pickupSlotDuration} MINUTE) > ?",
                            [$startTime]
                        );
                });
            })
     ...
     */
    private function getDonorPickupAddress(MatchRecord $match): string
    {
        return $match->donation->pickup_address;
    }

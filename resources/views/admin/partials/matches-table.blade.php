            ...
            <tbody>
                @forelse ($matches as $match)
                    <tr class="border-t border-gray-100">
                        ...
                        <td class="px-4 py-3">#{{ $match->donation_id }} ({{ $match->donation?->category->value ?? '—' }})</td>
                        <td class="px-4 py-3">#{{ $match->request_id }}</td>
                        <td class="px-4 py-3 capitalize">{{ $match->status->value }}</td>
                        <td class="px-4 py-3">{{ $match->matched_at?->format('M j, Y') ?? '—' }}</td>
                    </tr>
                @empty

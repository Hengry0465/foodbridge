...
<section>
    ...
    <p class="text-gray-500 text-sm mb-6">
        ...
        @else
            Cached stats
            @if ($platformStat)
                · last updated {{ $platformStat->created_at?->diffForHumans() }}
                · period {{ $platformStat->period_start->format('M j, H:i') }} – {{ $platformStat->period_end->format('M j, H:i') }}
            @endif
        @endif
    </p>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div class="bg-white rounded-xl shadow p-5 border border-gray-100">
            ...
            <p class="text-xs text-gray-400 mt-2">{{ $metrics['users']['active'] ?? '—' }} active · {{ $metrics['users']['inactive'] ?? '—' }} inactive</p>
        </div>
        <div class="bg-white rounded-xl shadow p-5 border border-gray-100">
            <p class="text-3xl font-bold text-emerald-700">{{ $metrics['donations']['total'] ?? $counts['donations'] }}</p>
    ...
    <div class="grid md:grid-cols-2 gap-4 mb-8">
        ...
        <div class="bg-white rounded-xl shadow p-5 border border-gray-100">
            ...
            <ul class="space-y-2 text-sm text-gray-600">
                <li>Meals redistributed: <span class="font-medium">{{ $metrics['impact']['meals_redistributed'] ?? '—' }}</span></li>
                <li>Food saved (kg): <span class="font-medium">{{ $metrics['impact']['food_kg_saved'] ?? '—' }}</span></li>
            </ul>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow p-5 border border-gray-100">
        ...
        <div class="flex flex-wrap gap-3">
            @foreach (['users', 'donations', 'requests', 'matches', 'pickups', 'reports'] as $tab)
                <a href="{{ route('admin.dashboard', ['tab' => $tab]) }}" class="text-emerald-700 hover:underline capitalize">{{ $tab }}</a>
            @endforeach
        </div>
    </div>
</section>

            ...
            <tbody>
                @forelse ($donations as $donation)
                    <tr class="border-t border-gray-100">
                        ...
                        <td class="px-4 py-3">{{ $donation->category?->name ?? '—' }}</td>
                        <td class="px-4 py-3">{{ $donation->quantity }} {{ $donation->unit }}</td>
                        <td class="px-4 py-3 capitalize">{{ str_replace('_', ' ', $donation->status) }}</td>
                        <td class="px-4 py-3">{{ $donation->created_at->format('M j, Y') }}</td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-gray-500">No donations found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>

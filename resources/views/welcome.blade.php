 ...
 <body class="w-full bg-white">
  <header class="w-full">
   <nav class="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
    ...
    <div class="flex items-center gap-3">
     @auth
      <a href="{{ match(auth()->user()->role->value) {
          'admin' => route('admin.dashboard'),
          'recipient' => route('recipient.dashboard'),
          default => route('donor.dashboard'),
      } }}" class="px-5 py-2.5 rounded-full font-medium text-emerald-800 border border-emerald-200 hover:bg-emerald-50 transition">
        Dashboard
      </a>
     @else
      <a href="{{ route('login') }}" class="px-5 py-2.5 rounded-full font-medium text-emerald-800 hover:bg-emerald-50 transition">
        Sign In
      </a>
      <a href="{{ route('register') }}" class="px-5 py-2.5 rounded-full font-medium text-white bg-emerald-700 hover:bg-emerald-800 transition">
        Sign Up
      </a>
     @endauth
    </div>
   ...
   <section class="relative w-full overflow-hidden">
    ...
    <div class="relative max-w-7xl mx-auto px-6 py-32 md:py-44 text-center text-white">
     ...
     <a href="{{ route('register') }}" class="canva-button px-5 py-2.5 rounded-full font-medium transition hover:opacity-90 inline-block bg-emerald-700 text-white">
      Join FoodBridge</a>
    </div>
   </section>
  </header>
  ...
  <main>
   ...
   <section class="w-full py-20 px-6 text-center bg-emerald-800 text-white">
    ...
    <a href="{{ route('register') }}" class="bg-white text-emerald-800 px-8 py-3.5 rounded-full font-semibold text-lg transition hover:scale-105 inline-block">
     Sign Up Now
    </a>
   </section>
  </main>
